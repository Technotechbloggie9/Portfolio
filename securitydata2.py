...
class dataconstructor():
    def blacklist(self, uuidinfo):
        self.blacklst.append(uuidinfo)
    def onblacklist(self, uuidinfo):
        status = False
        if uuidinfo in self.blacklst:
            status = True
        return status
    def whitelist(self, uuidinfo):
        self.whitelst.append(uuidinfo)
    def reputation(self, R, a1, a2, a3, a4, a5, a6):
        R = R - R * (a1/6 + a2/6 + a3/6 + a4/6 + a5/6 + a6/6)
        return R
    def trust(self, uuidinfo, base, r, rnk):
        tr = base * r * rnk
        if tr < self.threshold:
            self.blacklist(uuidinfo)
        elif tr > self.upperlimit:
            self.blacklist(uuidinfo)
        else:
            pass
    ...
    def listwrite(self, blacklistfilename:str):
        with open(blacklistfilename, 'w') as bl:
            for listitem in self.blacklst:
                b.write('%s\n' % listitem)        
        ...
        while rowcount <= rows:
            ...
            if rn in range(509756,511484): #rangestart	rangeend
                ...
                for s in range(0, spoofrange):
                    ...
                    dupaddr = 1
                    volfact = uniform(0.0,1.0)
                    delfact = 0
                    injcont = uniform(0,.5)
                    known = uniform(0.0, 1.0)
                    anom = 0
                    baseline = randint(2,3)
                    rep = self.reputation(rep, dupaddr, volfact, delfact, injcont, known, anom)
                    rank = uniform(.3, .5)
                    self.trust(unq, baseline, rep, rank)
                    vvaluetable.append([unq, #0
                                        baseline, #1 baseline
                                        rep, #2 reputation
                                        rank, #3 rank
                                        dupaddr, #4 duplicateaddress
                                        volfact, #5 volumefactor
                                        delfact, #6 delay factor
                                        injcont, #7 injectioncontent
                                        known, #8 known attack pattern
                                        anom, #9 anomaly attack pattern
                                        uniform(.1,.5), #10 last traffic density
                                        timestamp1 #11
                                        ])
                    if self.onblacklist(unq):
                                     break
                    rowcount = rowcount + 1
...
def main():
    ...
    dcon.constructIIoTdata(43200, "iiot13.csv")
    dcon.constructVANETdata(57600, "vanet14.csv")
    print("done")
    pause()
